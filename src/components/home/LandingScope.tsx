...
export function LandingScope() {
    ...
    const capabilities = [
        ...
        { icon: LayoutPanelTop, text: TranslationKeys.scope_does_3 },
        ...
        <section className="py-20 bg-white">
            <div className="container mx-auto px-4">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <Badge variant="outline" className="mb-4">{forKey(TranslationKeys.scope_badge)}</Badge>
                    <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-6">
                        {forKey(TranslationKeys.scope_title_1)} <br />
                        <span className="bg-gradient-to-r bg-clip-text text-transparent from-[#91ebfb] to-primary">{forKey(TranslationKeys.scope_title_2)}</span>
                    </h2>
                    ...
                    <Card className="border-green-100 bg-primary/5">
                        <CardHeader>
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <CardTitle className="text-xl text-primary">{forKey(TranslationKeys.scope_does_title)}</CardTitle>
                            </div>
                            <p className="text-slate-600 text-sm">{forKey(TranslationKeys.scope_does_desc)}</p>
                        </CardHeader>
                        <CardContent>
                            <ul className="grid gap-4">
                                {capabilities.map((item, index) => (
                                    <li key={index} className="flex items-start gap-3">
                                        ...
                                        <span className="text-slate-700 font-medium">{forKey(item.text)}</span>
                                    </li>
                                ))}
                            </ul>
                        </CardContent>
                    ...
                    <Card className="border-slate-200 bg-slate-50">
                        <CardHeader>
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <CardTitle className="text-xl text-primary">{forKey(TranslationKeys.scope_limits_title)}</CardTitle>
                            </div>
                            <p className="text-slate-600 text-sm">{forKey(TranslationKeys.scope_limits_desc)}</p>
                        </CardHeader>
                        <CardContent>
                            ...
                            <ul className="space-y-4">
                                {limitations.map((item, index) => (
                                    <li key={index} className="flex items-center gap-3 opacity-75">
                                        ...
                                        <span className="text-slate-600 line-through decoration-slate-400">{forKey(item.text)}</span>
                                    </li>
                                ))}
                            </ul>
                        </CardContent>
