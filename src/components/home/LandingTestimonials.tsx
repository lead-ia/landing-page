...
export function LandingTestimonials() {
    ...
    const testimonials = [
        ...
        {
            text: TranslationKeys.testimonial_3_text,
            author: TranslationKeys.testimonial_3_author,
            role: TranslationKeys.testimonial_3_role
        }
    ];
        ...
        <section className="py-20 bg-white">
            <div className="container mx-auto px-4">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <Badge variant="outline" className="mb-4">{forKey(TranslationKeys.testimonials_badge)}</Badge>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {testimonials.map((item, index) => (
                        <Card key={index} className="border-slate-200 bg-white hover:shadow-md transition-all">
                            <CardContent className="p-8 flex flex-col h-full">
                                ...
                                <div className="flex items-center gap-4 mt-auto border-t border-slate-100 pt-4">
                                    ...
                                    <div>
                                        <p className="font-bold text-slate-900 text-sm">{forKey(item.author)}</p>
                                        <p className="text-xs text-primary font-medium">{forKey(item.role)}</p>
                                    </div>
